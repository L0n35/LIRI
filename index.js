  ...
  .then(answers => {
    ...
    if (answers.main === "Random") {
      ...
      if (c[0] === "Spotify") {
        ...
        spotify.search({ type: "track", query: c[1] }, function(err, data) {
          if (err) {
            return console.log("Error occurred: " + err);
          }
          console.log("=-=-=-=");
      ...
      if (c[0] === "OMDB") {
        axios
          .get("http://www.omdbapi.com/?apikey=trilogy&plot=full&t=" + c[1])
          .then(function(response) {
            console.log("=-=-=-=");
            console.log("Title: " + response.data.Title);
            console.log("Year: " + response.data.Year);
